...
## 비활성 직원 계정 삭제 개인정보 보호

- 비활성 직원 계정 삭제는 안전 삭제를 기본으로 한다.
- 삭제 시 로그인 세션과 초대/구독 token을 revoke하고, User와 HR profile의 개인정보를 익명화한다.
- 휴가, 근태, 감사 로그, 휴가 장부 기록은 업무 증적이므로 삭제하지 않는다.
- 삭제 전 이메일, 전화번호, 주민등록번호, 계좌번호, passwordHash, tokenHash, codeHash는 AuditLog/Notification/export에 저장하지 않는다.
- 삭제된 직원은 화면에서 `삭제된 직원`으로 표시하고 기본 직원 목록에서는 숨긴다.
