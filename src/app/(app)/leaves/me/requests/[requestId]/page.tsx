...
import { canRequesterCancelApprovedLeaveRequest } from "@/lib/leave/cancellation";
...
const ERROR_MESSAGES: Record<string, string> = {
  ...
  "not-approved": "승인 완료 상태의 휴가만 취소할 수 있습니다.",
  "already-started": "이미 시작일이 지났거나 오늘 시작되는 휴가는 취소할 수 없습니다.",
...
export default async function LeaveRequestDetailPage({
  ...
  const canCancelApprovedLeave = canRequesterCancelApprovedLeaveRequest({
    leaveRequest,
  });
  ...
  return (
    <section className="max-w-3xl">
      ...
      {canCancelApprovedLeave ? (
        <form
          action={cancelMyApprovedLeaveRequest}
          className="mt-4 grid gap-3 rounded-lg border border-red-100 bg-white p-4 shadow-sm"
        >
          <input name="requestId" type="hidden" value={leaveRequest.id} />
          <label className="grid gap-1 text-sm font-medium">
            취소 사유
          ...
          <button className="h-10 w-full rounded-md border border-red-200 px-4 text-sm font-medium text-red-700 md:w-44">
            승인 휴가 취소
          </button>
        </form>
      ) : null}
