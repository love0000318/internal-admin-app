...
export default async function InvitationsAcceptPage({
  ...
  const isUsed =
    ...
    Boolean(invitation?.usedAt);
  const unavailable =
    (!token && !shortToken) ||
    !invitation ||
    invitation.status !== "PENDING" ||
    isInvitationExpired(invitation.expiresAt) ||
    (shortToken ? !validateInvitationShortTokenRecord(invitation).ok : false);
  ...
  return (
    <main className="flex min-h-screen items-center justify-center overflow-x-hidden bg-slate-100 px-4 py-6 text-slate-950 sm:py-10">
      <section className="w-full max-w-xl rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
